...
const loadDoctors = (search) =>{
  ...
  console.log(search)
  fetch(` https://test-thto.onrender.com/doctor/list/?search=${
        search ? search : ""
      }`)
  .then((res)=> res.json())
  .then((data)=>{  
  ...
  doctors?.forEach((doctor)=>{
    ...
    div.innerHTML = `
      <img class="doc-img" src=${doctor.image} alt="" />
        ...
        <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Nobis,numquam!</p>
        <p>${doctor?.specialization?.map((item) => {return `<button>${item}</button>`;})}</p>
